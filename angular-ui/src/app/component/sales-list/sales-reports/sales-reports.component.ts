...
export class SalesReportsComponent implements OnInit {
  ...
  image1 = 'assets/image/download-disable.png';
  columnDefs: ColDef[] = [
    // { headerName: "User Id",
  ...
  customDatePickerEvent(eventChange) {
    ...
    this.selectedFilters.startdate = this.selectedDateRange.startDate;
    this.selectedFilters.enddate = this.selectedDateRange.endDate;
  }

  ngOnInit(): void {
    this.loggedUserId = localStorage.getItem('logInId');
    this.selectedFilters.currentUserId = this.loggedUserId;
  ...
  geographyData() {
    this.geoGraphyList = [];
    let req: any = {};
    req.customerId = this.selectedFilters.dealerIds; // dealer
    req.CurrentUserId = this.loggedUserId;
  ...
  getProductData() {
    let req: any = {};
    req.customerId = this.selectedFilters.dealerIds; // dealer
    req.GeographyId = this.selectedFilters.geographieIds;
    req.CurrentUserId = this.loggedUserId;
    this.productListData = [];
    this.reportsService.postMProductdropbyDealeridGeoid(req).subscribe((res) => {
  ...
  getInvoicetranscationDropdownData() {
    this.reportList = [];
    this.reportsService.getInvoicetranscationDropdown(this.loggedUserId).subscribe((res) => {
      console.log(res.response);
  ...
  onItemSelected(item: any, type) {
    ...
    switch (type) {
      ...
      case 'product': {
        this.selectedFilters.productIds.push(item.stockItemId);
        // this.selectedFilters.GeographyId =[];
        // this.selectedFilters.customerId.push(item.customerid);
  ...
  onItemDeSelected(item: any, type) {
  ...
  onItemSelectOrAllProduct(items: any, type) {
  ...
  onItemDeSelectOrAllProduct(item: any, type) {
    switch (type) {
      case 'dealer': {
        this.selectedFilters.geographieIds = [];
        this.selectedFilters.productIds = [];
  ...
  onClickAdvancedFilter() {
    this.dialog.open(AdvancedFilterComponent, { width: '800px', height: '623px' });
  }
}
