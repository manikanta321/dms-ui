...
export class TargetGroupsComponent implements OnInit {
  ...
  columnDefs: ColDef[] = [
...

    {
      ...
      tooltipValueGetter: (params: ITooltipParams) =>
        moment(params.value).format('DD MMM YY, HH:mm A'),
    },

    {
      headerName: 'No of Geographies',

      field: 'noOfGeographies',
      cellStyle: { color: '#686E74' },
      type: ['rightAligned'],
      maxWidth: 200,
    },
  ...
  toogleStatusFilter() {
    ...
    this.dropdownSettings1 = Object.assign({}, this.dropdownSettings1, {
  ...
  TargetListData() {
    ...
    this.targetList.getTargetSearch(data).subscribe((res) => {
      ...
      this.targetlistData = dataCat.map(
        (data: { targetGroupId: any; targetGroupName: any }) => {
          return {
            targetGroupId: data.targetGroupId,
            roleName: data.targetGroupName,
          };
        }
      );
